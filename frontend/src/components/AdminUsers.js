...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const normalized = snapshot.docs.map((docSnapshot) => {
        ...
        const role = (data.role || data.userType || "student").toString().toLowerCase();
  ...
  const syncClassAssignments = async (
    ...
    const normalizedPreviousRole = (previousRole || normalizedNewRole).toLowerCase();
    ...
    const unchanged = newClasses.filter((id) => previousClasses.includes(id));
    ...
    const applyUpdate = async (classId, role, operation) => {
      ...
      if (role === "teacher") {
        if (operation === "add") {
          ...
          const currentTeacher = snapshot.exists() ? snapshot.data().teacher : "";

          const updates = [];

          if (currentTeacher && currentTeacher !== userId) {
            updates.push(updateDoc(doc(db, "users", currentTeacher), { classes: arrayRemove(classId) }));
          }
      ...
      if (role === "student") {
        return updateDoc(classRef, {
          students: operation === "add" ? arrayUnion(userId) : arrayRemove(userId),
        });
      }
      return Promise.resolve();
    };

    const classUpdates = added.map((classId) => applyUpdate(classId, normalizedNewRole, "add"));
    const removalUpdates = removed.map((classId) => applyUpdate(classId, normalizedPreviousRole, "remove"));
    ...
    await Promise.all([...classUpdates, ...removalUpdates, ...roleChangeUpdates]);
  };
